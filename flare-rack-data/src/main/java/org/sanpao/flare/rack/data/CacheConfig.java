...
import org.sanpao.flare.rack.data.store.RackAdvertisementStore;
import org.sanpao.flare.rack.domain.entity.RackAdvertisement;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;

@Configuration("rackCacheConfig")
@EnableIgniteRepositories({ "org.sanpao.flare.rack.data.cao" })
public class CacheConfig {
	...
	@Bean
	public CacheConfiguration<Long, RackAdvertisement> shelfInfoCacheConfig() {
		CacheConfiguration<Long, RackAdvertisement> cacheConfig = new CacheConfiguration<Long, RackAdvertisement>(CacheNames.RACK_ADVERTISEMENT_CACHE_NAME);
		// Setting SQL schema for the cache.
		cacheConfig.setIndexedTypes(String.class, RackAdvertisement.class);
		...
		cacheConfig.setCacheStoreFactory(FactoryBuilder.factoryOf(RackAdvertisementStore.class));
		return cacheConfig;
	}

	@Bean
	public List<CacheConfiguration<?, ?>> shelfCacheConfigurations() {
		List<CacheConfiguration<?, ?>> cacheConfigurations = new ArrayList<CacheConfiguration<?, ?>>();
		cacheConfigurations.add(shelfInfoCacheConfig());
	...
	public Ignite igniteInstance() {
		...
		configuration.setCacheConfiguration(new CacheConfiguration[] { shelfInfoCacheConfig() });
