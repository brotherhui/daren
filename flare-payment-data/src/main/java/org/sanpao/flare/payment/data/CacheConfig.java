...
import org.sanpao.flare.payment.data.store.PaymentInfoStore;
import org.sanpao.flare.payment.domain.entity.PaymentInfo;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;

@Configuration("paymentCacheConfig")
@EnableIgniteRepositories({ "org.sanpao.flare.payment.data.cao" })
public class CacheConfig {
	...
	@Bean
	public CacheConfiguration<Long, PaymentInfo> paymentInfoCacheConfig() {
		CacheConfiguration<Long, PaymentInfo> cacheConfig = new CacheConfiguration<Long, PaymentInfo>(CacheNames.PAYMENT_INFO_CACHE_NAME);
		// Setting SQL schema for the cache.
		cacheConfig.setIndexedTypes(String.class, PaymentInfo.class);
		...
		cacheConfig.setCacheStoreFactory(FactoryBuilder.factoryOf(PaymentInfoStore.class));
		return cacheConfig;
	}

	@Bean
	public List<CacheConfiguration<?, ?>> paymentCacheConfigurations() {
		List<CacheConfiguration<?, ?>> cacheConfigurations = new ArrayList<CacheConfiguration<?, ?>>();
		cacheConfigurations.add(paymentInfoCacheConfig());
	...
	public Ignite igniteInstance() {
		...
		configuration.setCacheConfiguration(new CacheConfiguration[] { paymentInfoCacheConfig() });
